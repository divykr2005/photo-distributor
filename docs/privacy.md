...
## 5. Biometric Data Retention & Automated Purging

- **Consent Recording:** When a guest registers and uploads a selfie, their explicit consent is recorded via `consent_given_at`, `consent_source`, and `consent_text_version`.
- **Face Embeddings:** The uploaded selfie is converted to a mathematical embedding to find matches. The raw image is retained only for potential re-extraction if the underlying face model upgrades, and is strictly access-controlled.
- **Automated Expiration:** By default, guest biometric profiles expire 15 days after creation (`expires_at` column).
- **Sweep Job:** A scheduled Celery beat task (`workers.maintenance.sweep_expired_guests`) runs daily to permanently hard-delete expired guests from the database, which cascades to remove all associated `PhotoMatch` records and physically deletes their uploaded selfie from storage.
- **Right to Erasure:** Event organizers can manually delete a guest record via the Organizer Dashboard at any time, which instantly enforces the cascade deletion and storage purge mentioned above.
