...
def _get_or_create_magic_token(db: Session, guest: Guest) -> tuple[str, str | None]:
    """Ensure guest has an active GuestAccessToken, returns (token_prefix, raw_token_if_created)."""
    token_record = (
    ...
    if token_record:
        return str(token_record.token_prefix), None
    ...
    db.add(token_record)
    db.commit()
    return str(token_record.token_prefix), raw_token


def _rotate_magic_token(db: Session, guest: Guest) -> tuple[str, str]:
    """Revoke existing tokens and generate a new one. Returns (token_prefix, raw_token)."""
    db.query(GuestAccessToken).filter(
        GuestAccessToken.guest_id == guest.id,
        GuestAccessToken.revoked_at.is_(None)
...
def dispatch_guest_notification(
    ...
    try:
        ...
        prefix, db_raw_token = _get_or_create_magic_token(db, guest)
        
        # If we weren't passed a raw_token, and we didn't just create one, we must rotate to send an email.
        if not raw_token:
            if db_raw_token:
                raw_token = db_raw_token
            else:
                prefix, raw_token = _rotate_magic_token(db, guest)
        ...
        in_quiet, next_available_utc = is_in_quiet_hours(now_utc, str(event.timezone) if event.timezone else "UTC")
        if False: # TEMPORARILY DISABLED: in_quiet and next_available_utc and not force and not celery_app.conf.task_always_eager:
        ...
        subject, text_body, html_body = render_email_template(
            guest_name=str(guest.first_name),
        ...
        existing_log.status = NotificationStatus.SENDING.value  # type: ignore
        db.commit()
        ...
        # 7. Update log result
        if res.success:
            existing_log.status = NotificationStatus.SENT.value  # type: ignore
            existing_log.provider = res.provider  # type: ignore
            existing_log.provider_message_id = res.provider_message_id  # type: ignore
            existing_log.sent_at = datetime.now(timezone.utc)  # type: ignore
            existing_log.error = None  # type: ignore
            guest.last_notified_at = datetime.now(timezone.utc)  # type: ignore
            db.commit()
            ...
            if res.is_transient and int(existing_log.attempts or 0) < 5:
                ...
                dispatch_guest_notification.apply_async(
                    args=[guest_id, channel, notification_type, force, raw_token],
...
def run_event_notification_dispatch(
    ...
    for guest in guests:
        ...
        # Check photos
        photo_count = visible_matches(db, str(guest.id)).count()  # type: ignore
        if photo_count == 0:
